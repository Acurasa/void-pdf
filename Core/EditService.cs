...
namespace YourBlazorApp.Services
{
    ...
    {
        public MemoryStream HighlightText(Stream pdfStream, double x, double y, double width, double height,
            int pageIndex)
        {
            // Открываем PDF документ из потока
            PdfDocument document = PdfReader.Open(pdfStream, PdfDocumentOpenMode.Modify);

            // Получаем страницу для выделения
            if (pageIndex >= 0 && pageIndex < document.Pages.Count)
            {
                ...
                // Используем белую кисть без прозрачности
                XSolidBrush brush = new XSolidBrush(XColors.White);
